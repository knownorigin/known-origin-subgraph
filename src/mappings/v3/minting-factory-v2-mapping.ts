...
} from "../../../generated/MintingFactoryV2/MintingFactory";
...
export function handleEditionMintedAndListed(event: EditionMintedAndListed): void {
    ...
    editionMinted(event, event.params._editionId);
}

export function handleEditionMinted(event: EditionMinted): void {
    log.info("Minting factory - handleEditionMinted() called - edition ID {}", [
        event.params._editionId.toString()
    ]);
    editionMinted(event, event.params._editionId);
}

function editionMinted(event: ethereum.Event, editionId: BigInt): void {
    ...
    let edition = editionService.loadOrCreateV3Edition(editionId, event.block, contract)
    edition.save()
